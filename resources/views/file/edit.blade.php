...
@section('content')
                    ...
                    @endif
                        <form class="form-horizontalx" method="POST" action="{{route('file.docs.edit', ['id'=>$data->id])}}"  enctype="multipart/form-data">
                            {{csrf_field()}}
                            {{ method_field('POST') }}
                            <div class="row clearfix">
                                ...
                                <div class="col-sm-4">
                                    ...
                                    <select name="subcat_id" id="subcat_id" class="form-control select">
                                        @php $_subcat_id = !empty(old('subcat_id')) ? old('subcat_id') : $data->subcat_id @endphp
        ...
        $('#cat_id').on('change', function () {
            ...
            $.get(url, function (data) {
                ...
                $.each(data.sub, function () {
                    if ("{!! $_subcat_id !!}"==this['id']) {
                        $('#subcat_id').append('<option value="' + this['id'] + '" selected>' + this['name'] + '</option>');
